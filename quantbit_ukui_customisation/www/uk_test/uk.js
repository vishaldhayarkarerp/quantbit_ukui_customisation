...
function debugCredentials() {
    ...
    if (!API_SECRET || API_SECRET === "{{ api_secret }}" || API_SECRET === "") {
        console.error('API Secret is not properly set');
        return false;
    }
    return true;
}
                    ...
                    if (file instanceof File) {
                        try {
                            ...
                            if (fileDocResponse.ok) {
                                console.log(`File ${file.name} attached to record ${recordName}`);
                            } else {
                                console.error(`Failed to attach file ${file.name}:`, await fileDocResponse.json());
                            }
