...
// API Configuration
const FRAPPE_API_BASE = 'http://103.219.1.138:4426/api/resource';
...
async function saveToFrappe(formData) {
    try {
        ...
        async function getCSRFToken() {
            ...
            try {
                const response = await fetch('http://103.219.1.138:4426/api/method/frappe.auth.get_csrf_token', {
                    method: 'GET',
                    credentials: 'include',
                    headers: {
        ...
        if (result.data && result.data.name) {
            setTimeout(() => {
                window.location.href = `http://103.219.1.138:4426/app/medical-assessment/${result.data.name}`;
            }, 1500);
        }
